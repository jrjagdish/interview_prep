...
export default function AdminPanel() {
  ...
  const fetchCandidates = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:8000/api/admin/guest-candidates/",
      );
  ...
  const handleGenerateInvite = async () => {
    try {
      ...
      const response = await fetch("http://localhost:8000/admin/invite-link", {
        method: "POST",
        credentials:"include"
      });
      const data = await response.json();
  ...
  const handleDeleteCandidate = async (id) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/admin/guest-users/${id}`,
        { method: "DELETE" },
      );
      if (response.ok) {
        ...
        <aside className="hidden lg:flex flex-col w-64 border-r border-slate-200 dark:border-white/5 bg-white dark:bg-[#05080a] p-6 sticky top-0 h-screen">
          <Link to="/" className="flex items-center gap-2.5 mb-10 px-2 group">
            <div className="relative">
              ...
              <BrainCircuit
                size={10}
                className="absolute -top-1 -right-1 text-purple-500 bg-white dark:bg-[#05080a] rounded-full"
              />
            </div>
          ...
          <div className="mt-auto p-4 bg-slate-100 dark:bg-purple-600/5 rounded-2xl border border-slate-200 dark:border-purple-500/10">
            <p className="text-[10px] text-slate-500 dark:text-purple-400/60 font-black uppercase tracking-widest">
              System Status
            </p>
            <div className="mt-2 flex items-center gap-2">
              <div className="size-2 bg-emerald-500 rounded-full animate-pulse" />
              <span className="text-xs font-bold text-emerald-600 dark:text-emerald-500/90">
                Node Online
              </span>
            </div>
          </div>
          ...
          <header className="sticky top-0 z-40 flex items-center justify-between p-4 border-b border-slate-200 dark:border-white/5 bg-white/80 dark:bg-[#05080a]/80 backdrop-blur-xl">
            <div className="flex items-center gap-3">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-[9px] font-bold text-purple-600 uppercase tracking-tighter mt-1">
                  Super Admin
                </p>
              </div>
            </div>
          ...
          <main className="p-6 lg:p-8 max-w-6xl w-full mx-auto space-y-8">
            ...
            <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-3xl overflow-hidden shadow-sm">
              <div className="flex items-center justify-between px-6 py-5 border-b border-slate-200 dark:border-white/5">
                <h2 className="font-bold dark:text-white text-sm">
                  Review Queue
                </h2>
                {loading && (
              ...
              <div className="divide-y divide-slate-100 dark:divide-white/5">
                {candidates.length > 0 ? (
                  candidates.map((user) => (
                    <div
                      key={user.id}
                      className="flex flex-col sm:flex-row sm:items-center justify-between p-4 sm:px-6 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors gap-4"
                    >
                      <div className="flex items-center gap-3">
                        <img
                          src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${user.id}`}
                          className="size-10 rounded-xl bg-slate-100 dark:border-white/10 dark:bg-slate-800"
                          alt="Avatar"
                        />
                        <div>
...
function NavItem({ icon, label, active = false }) {
  return (
    <div
      className={`flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer transition-all ${active ? "bg-purple-600 text-white shadow-md shadow-purple-600/20" : "text-slate-500 hover:bg-slate-100 dark:hover:bg-white/5 dark:hover:text-white"}`}
    >
