...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        // Updated to use the profile endpoint we discussed
        ...
        <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
          <section className="lg:col-span-1 flex flex-col items-center lg:items-start text-center lg:text-left space-y-6">
            <div className="relative">
              ...
              <div className="relative w-32 h-32 rounded-3xl bg-gradient-to-tr from-purple-600 to-fuchsia-400 p-[3px]">
                <div
                  ...
                  style={{
                    backgroundImage: `url(https://api.dicebear.com/7.x/avataaars/svg?seed=${user?.user_info?.username})`,
                  }}
                />
              </div>
            ...
            <div className="space-y-1">
              <h2 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">
                {user?.user_info?.username || "Engineer"}
              </h2>
              <p className="text-purple-600 dark:text-purple-400 font-semibold flex items-center justify-center lg:justify-start gap-2">
                {userImg ? (
                  <img
                    src={userImg}
                    alt="Profile"
                    className="size-5 rounded-full object-cover border border-purple-500/20"
                    onError={(e) => {
          ...
          <section className="lg:col-span-2 space-y-8">
            <div className="grid grid-cols-3 gap-4">
              {[
                // Using interview_credits from profile and total_interviews from stats
                {
                  label: "Credits",
                  val: user?.profile_info?.interview_credits ?? "0",
                  color: "text-purple-500",
                  bg: "bg-purple-500/10",
                },
                {
                  label: "Interviews",
                  val: user?.stats?.total_interviews ?? "0",
                ...
                <div
                  key={i}
                  className={`${s.bg} p-4 rounded-2xl border border-white/5`}
                >
            ...
            <div className="space-y-4">
              ...
              {/* Static placeholders since activity wasn't in the specific SQL query yet */}
              {[
                {
                  title: "Technical Mock: React & Node",
                  date: "2 days ago",
                  score: "88%",
                },
                {
                ...
                <div
                  key={i}
                  className="flex items-center justify-between p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl hover:border-purple-500/50 transition-colors cursor-pointer group"
                >
