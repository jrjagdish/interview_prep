...
export default function Dashboard() {
  ...
  const handelHistory = async (e) => {
    try {
      const response = await fetch("http://localhost:8000/interview/history", {
        method: "GET",
        credentials: "include",
      });
      const data = await response.json();
      setHistory(data);
      alert(`data recieved ${history?.interview_id}`);
    } catch (err) {
  ...
  const recommendations = [
    ...
    },
    {
      ...
      icon: <Palette size={20} className="text-fuchsia-500" />,
    },
    {
            ...
            <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[1rem] p-8 md:p-10 text-white shadow-2xl">
              <div className="relative z-10 space-y-4 max-w-md">
                ...
                <h2 className="text-4xl md:text-5xl font-black leading-tight tracking-tighter">
                  Start AI Voice <br /> Interview
                </h2>
                <p className="text-purple-100 opacity-90 leading-relaxed text-lg font-medium">
                  Master your technical communication with our GPT-powered voice
                  agent.
                </p>
                <button
              ...
              <Mic
                size={280}
                className="absolute -right-16 -bottom-16 text-white/10 rotate-12 pointer-events-none opacity-20"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <Link to="/code-editor">
                  <div
                    key={i}
                    className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-[1rem] hover:border-purple-500/50 transition-all cursor-pointer group shadow-sm"
                  >
                    <div
                      className={`${cat.color}/10 ${cat.color.replace("bg-", "text-")} size-14 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}
                    >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1rem] p-8 shadow-sm">
              <h3 className="font-bold mb-6 flex items-center gap-2">
                <Sparkles size={18} className="text-purple-500" /> Latest
                Insights
              </h3>
              <div className="space-y-5">
                <div className="flex gap-4 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-transparent hover:border-purple-500/20 transition-all cursor-pointer group">
                  <div className="size-10 shrink-0 rounded-xl bg-purple-600 text-white flex items-center justify-center font-bold text-xs">
                    AI
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm font-bold group-hover:text-purple-600 transition-colors">
                      Communication Feedback
                    </p>
                    <p className="text-[11px] text-slate-500">
                      Clarity & Confidence score improved.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <section className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1rem] p-8 shadow-sm">
          <div className="flex justify-between items-center mb-10">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm">
                Your personalized path to Senior Software Engineer
              </p>
            </div>
          ...
          <div className="relative flex items-center justify-between px-4">
            ...
            <div className="relative z-10 flex flex-col items-center gap-3">
              <div className="size-12 bg-purple-600 text-white rounded-full flex items-center justify-center shadow-lg">
                <Check size={20} />
              </div>
              <div className="text-center">
                <p className="font-bold text-sm">Junior Dev</p>
                <p className="text-[10px] text-slate-400">Completed</p>
              </div>
            </div>
            <div className="relative z-10 flex flex-col items-center gap-3">
              <div className="size-14 bg-purple-600 text-white rounded-full flex items-center justify-center shadow-xl ring-4 ring-purple-500/20">
                <Sparkles size={24} />
              </div>
              <div className="text-center">
                <p className="font-bold text-sm text-purple-600">Mid-Level</p>
            ...
            <div className="relative z-10 flex flex-col items-center gap-3">
              <div className="size-12 bg-slate-100 dark:bg-slate-800 text-slate-400 rounded-full flex items-center justify-center">
                <Lock size={20} />
              </div>
              <div className="text-center">
                <p className="font-bold text-sm text-slate-500">Senior</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-10">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1rem] p-8 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold flex items-center gap-2">
                <History size={20} className="text-purple-600" /> Interview
                History
              </h3>
              <button
            ...
            <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
              {[
                ...
                <div
                  key={i}
                  className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-transparent hover:border-purple-500/20 transition-all flex justify-between items-center"
                >
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-[1rem] p-8 shadow-sm">
            <h3 className="text-lg font-bold flex items-center gap-2 mb-6">
              <BookOpen size={20} className="text-purple-600" /> Daily Code News
            </h3>
            <div className="space-y-6">
              <div className="flex gap-4 group cursor-pointer">
                <div className="size-20 shrink-0 rounded-2xl bg-slate-100 dark:bg-slate-800 overflow-hidden">
                  <img
                    src="https://api.dicebear.com/7.x/shapes/svg?seed=rust"
                    alt="news"
                ...
                <div>
                  ...
                  <h4 className="font-bold text-sm leading-tight group-hover:text-purple-600 transition-colors">
                    Rust vs. Go: 2026 Performance Benchmarks
                  </h4>
                  <p className="text-[11px] text-slate-500 mt-1 line-clamp-2">
                    New memory safety protocols changing microservice
                    architecture...
                  </p>
                </div>
              </div>
